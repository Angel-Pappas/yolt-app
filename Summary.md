...
- **Database**: Supabase Postgres. Project `yolt-app`, ref `mzfxfweljbfvyqlhvmzr`, region `eu-west-1`, free tier, org "Angel-Pappas's Org" (`yowsydnsxjwcujsjwtji` — same org as the Exalted-Character-App project).
  ...
  - **Soft delete**: the app's "Delete" action does `update is_deleted = true, deleted_at = now()`, never a real `DELETE`. The SELECT RLS policy itself filters out `is_deleted = true` rows (`Users can view their own non-deleted transactions`), so soft-deleted rows can't leak into the app even if a query forgets to filter — the app query filters on `is_deleted` too, redundantly, for clarity. There is **no DELETE policy** on the table at all anymore (dropped deliberately) — the app has no way to hard-delete a row, by design. Only the user (owner) can ask for a permanent delete or a restore, done by directly running SQL via the Supabase MCP (`execute_sql`/`apply_migration`), not through any in-app feature.
